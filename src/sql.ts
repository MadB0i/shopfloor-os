...
function wrapPglite(db: PGlite): SqlPool {
...
/** `pglite:memory` is in-RAM (tests). `pglite:./data/shopfloor` is on disk. */
export function createPoolFromUrl(url: string): SqlPool {
  if (url.startsWith("postgres://") || url.startsWith("postgresql://")) {
    return postgresPool(url);
  }
  if (url === "pglite:memory") {
    return wrapPglite(new PGlite());
  }
  if (url.startsWith("pglite:")) {
    const dir = path.resolve(url.slice("pglite:".length).trim() || "./data/shopfloor");
    fs.mkdirSync(path.dirname(dir), { recursive: true });
    return wrapPglite(new PGlite(dir));
  }
  throw new Error("DATABASE_URL must be postgres://... or pglite:...");
}

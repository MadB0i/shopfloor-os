...
export async function effectiveQtySum(
  ...
  scope?: { assetId?: string; workOrderId?: string; from?: Date; to?: Date },
) {
  const dead = await supersededEventIds(db, plantId);
  const { rows } = await db.query<{ event_id: string; payload: unknown; asset_id: string | null; work_order_id: string | null; occurred_at: Date }>(
    `SELECT event_id, payload, asset_id, work_order_id, occurred_at FROM floor_events WHERE plant_id = $1 AND type = $2`,
    [plantId, type],
  );
  let sum = 0;
  for (const row of rows) {
    ...
    if (scope?.workOrderId && row.work_order_id !== scope.workOrderId) continue;
